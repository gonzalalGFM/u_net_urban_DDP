    ...
    def __init__(self, cuda_device_idx: int):
        ...
        try:
            ...
            for i in range(pynvml.nvmlDeviceGetCount()):
                ...
                if nvml_uuid.lower().removeprefix("gpu-") == target_uuid_norm:
                    self._nvml_handle = h
                    break
        except Exception:
            pass
...
def parse_args():
    p = argparse.ArgumentParser(description="DDP UNet training with optional torch.compile + AMP.")
    p.add_argument("--lr", type=float, default=1e-4)
    p.add_argument("--base-channels", type=int, default=32)
    p.add_argument("--batch-size", type=int, default=16, help="Per-GPU batch size")
    p.add_argument("--num-workers", type=int, default=16)
    p.add_argument("--compile", action="store_true", help="Enable torch.compile on the DDP model")
    p.add_argument("--compile-mode", default="default", choices=["default", "reduce-overhead", "max-autotune"])
    p.add_argument(
        "--amp-dtype",
        default="fp16",
        choices=["none", "fp16", "bf16"],
        help="AMP dtype; use 'none' to disable autocast.",
    )
...
def main():
    ...
    if rank == 0:
        print(
            f"[DDP-COMPILE] ranks={world_size} lr={args.lr} base_ch={args.base_channels} "
            f"batch/gpu={args.batch_size} workers={args.num_workers} "
            f"compile={args.compile} mode={args.compile_mode} amp={args.amp_dtype}"
        )
    ...
    if rank == 0:
        print(f"[DDP-COMPILE] train={len(train_ds)} val={len(val_ds)}")
    ...
    if args.compile:
        # Compile after wrapping with DDP, following your chapter-style pattern.
    ...
    for epoch in range(EPOCHS):
        ...
        if rank == 0:
            ...
            print(
                f"[DDP-COMPILE] {epoch+1}/{EPOCHS} train_loss={train_loss:.6f} "
        ...
        if val_loss < best_val:
            ...
            patience = 0
            if rank == 0:
                torch.save(model.module.state_dict(), output_dir / "best_unet_ddp_compile.pth")
        else:
            patience += 1
        if patience >= PATIENCE:
            if rank == 0:
                print("[DDP-COMPILE] Early stopping triggered.")
            break

    if rank == 0:
        np.savez(
            output_dir / "history_ddp_compile.npz",
        ...
        print("\n" + "=" * 60)
        print("  DDP-COMPILE Training Summary")
        print("=" * 60)
        if gpu_utils:
        ...
        print("=" * 60)
        print(f"  Best model  ->  {output_dir / 'best_unet_ddp_compile.pth'}\n")
